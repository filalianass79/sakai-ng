...
@Component({
  selector: 'app-list-societes-archives',
  standalone: true,
  imports: [
    CommonModule,
    ButtonModule,
    ...
    <div class="bg-surface-50 dark:bg-surface-900 p-12 min-h-screen">
      <div class="max-w-8xl mx-auto">
        <p-card styleClass="shadow">
          <h1 class="text-3xl font-bold text-surface-900 dark:text-surface-0">Liste des Sociétés archivées</h1>
          ...
          <p-toolbar styleClass="mb-12">
            <ng-template pTemplate="left">
              <p-button 
                ...
                icon="pi pi-trash" 
                [outlined]="true"
                (onClick)="deleteSelectedSocietes()" 
...
export class ListSocietesArchives implements OnInit {
  ...
  loadSocietes() {
    this.loading = true;
    this.societeService.getArchivesSocietes().subscribe({
      next: (data) => {
        this.societes = data;
        this.loading = false;
      },
  ...
  activeSociete(societe: Societe) {
    if (societe.id) {
      this.loading = true;
      this.societeService.activeSociete(societe.id).subscribe({
        next: () => {
          this.messageService.add({
            severity: 'success',
            summary: 'Succès',
  ...
  confirmDelete(societe: Societe) {
    this.confirmationService.confirm({
      ...
      acceptLabel: 'Oui',
      rejectLabel: 'Non',
      accept: () => this.deleteSociete(societe)
    });
  }
